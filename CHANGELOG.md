...
- Initial `react-offline-first` implementation: `OfflineProvider`, `useNetworkStatus`, `useOfflineQuery`, `useMutation`, `useQueue`, `useConflict`
- Storage adapters: `MemoryAdapter`, `IndexedDBAdapter`, `LocalStorageAdapter`
- Service worker registration helper (`react-offline-first/sw`) and minimal Background Sync worker script
